...
* flexibility (does _not_ try to completely abstract away HTML, JavaScript or HTTP)
* direct access to form parameters
* composable form parts (like "formlets")
* effective measures against common security threads
* verifying of embedded JavaScript at compile time with JSLint
...
Build the web server with <code>build.sh</code> in the <code>sawhorse</code> directory. <b>You will need G++ installed. On Windows, you need Cygwin and G++ installed.</b><br>
...
There is one real problem with this example, though. The inner functions do not return valid HTML documents but only fragments. We could fix this by extending these functions with <code>html(head(title(...)) body(...))</code>. However, there is a better way to share code between multiple functions.
